...
exports.analyzeATS = exports.cleanResumeText = void 0;
...
function detectSections(text) {
    ...
    const hasProjects = SECTION_KEYWORDS.projects.some(kw => lower.includes(kw)) ||
        /\b(project|hackathon|case study)\b/i.test(text);
    const hasContact = EMAIL_REGEX.test(text) || PHONE_REGEX.test(text);
    const hasLinks = GITHUB_REGEX.test(text) ||
...
function scoreExperience(hasExperience, text) {
    if (!hasExperience)
        return 0;
...
// 0–15: education
function scoreEducation(hasEducation, text) {
    if (!hasEducation)
        return 0;
    ...
    if (hasMultipleDegrees || hasGpa)
        return 15;
    return 11;
}
// 0–15: role keyword relevance
function scoreKeywords(roleMatchCount) {
    if (roleMatchCount >= 6)
        return 15;
    if (roleMatchCount >= 4)
        return 12;
...
// 0–10: contact & links
function scoreContactLinks(hasContact, hasLinks) {
    if (hasContact && hasLinks)
        return 10;
    if (hasContact || hasLinks)
        return 6;
    return 0;
}
function buildSuggestions(sections, wordCount, missingKeywords, detectedSkills) {
    const suggestions = [];
    if (!sections.hasSkills)
        suggestions.push("Add a clearly titled Skills section with 8-15 targeted keywords.");
    if (!sections.hasExperience)
        suggestions.push("Include a Work Experience section with quantifiable bullet points.");
    if (!sections.hasEducation)
        suggestions.push("Add an Education section with degree and graduation year.");
    if (!sections.hasContact)
        suggestions.push("Add both email and phone number in the header for quick contact.");
    if (!sections.hasProjects)
        suggestions.push("Add 1-2 Projects with outcomes and links if available.");
    if (!sections.hasLinks)
        suggestions.push("Include LinkedIn and GitHub URLs for ATS parsers and recruiters.");
    if (wordCount < 320)
        suggestions.push("Expand impact statements; aim for 400-800 words for completeness.");
    else if (wordCount > 1100)
        suggestions.push("Condense wording; keep resume concise (400-800 words ideal).");
...
function analyzeATS(text) {
    ...
    const breakdown = {
        skillsMatch: scoreSkills(detectedSkills),
        experience: scoreExperience(sections.hasExperience, cleanedText),
        projects: scoreProjects(sections.hasProjects, cleanedText),
        education: scoreEducation(sections.hasEducation, cleanedText),
        keywords: scoreKeywords(matched),
    ...
    if (!sections.hasExperience)
        missingSections.push("Experience / Internship");
    if (!sections.hasEducation)
        missingSections.push("Education / Academic Background");
    if (!sections.hasProjects)
        missingSections.push("Projects");
    if (!sections.hasContact)
        missingSections.push("Contact Details");
    if (!sections.hasLinks)
        missingSections.push("Links (GitHub / LinkedIn)");
